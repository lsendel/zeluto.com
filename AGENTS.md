...
Never commit credentials; store Worker secrets via `wrangler secret put` and keep `.env*` local. Use the shared Neon branch (see `.dev.vars`) before running queue processors, and re-run `scripts/apply-rls.ts` whenever tables or policies change. Guard every query with the `tenantContext` helpers from `@mauntic/domain-kernel` to preserve isolation.
