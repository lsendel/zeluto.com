...
export const enrichmentProviders = leadIntelligenceSchema.table(
  'enrichment_providers',
  {
    ...
    cost_per_lookup: numeric('cost_per_lookup', { precision: 10, scale: 4 })
      .notNull()
      .default('0'),
    avg_latency_ms: integer('avg_latency_ms').default(0),
    success_rate: numeric('success_rate', { precision: 5, scale: 4 }).default(
      '0',
    ),
...
export const waterfallConfigs = leadIntelligenceSchema.table(
  'waterfall_configs',
  {
    ...
    min_confidence: numeric('min_confidence', { precision: 3, scale: 2 })
      .notNull()
      .default('0.5'),
    cache_ttl_days: integer('cache_ttl_days').notNull().default(7),
    max_cost_per_lead: numeric('max_cost_per_lead', {
      precision: 10,
      scale: 4,
    }),
    created_at: timestamp('created_at').notNull().defaultNow(),
...
export const enrichmentJobs = leadIntelligenceSchema.table(
  'enrichment_jobs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const enrichmentCache = leadIntelligenceSchema.table(
  'enrichment_cache',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
  ...
  (table) => ({
    lookupIdx: {
      columns: [table.organization_id, table.contact_id, table.field_name],
    },
    expiryIdx: { columns: [table.expires_at] },
  }),
);

export const providerHealth = leadIntelligenceSchema.table(
  'provider_health',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
    ...
    circuit_state: varchar('circuit_state', { length: 20 })
      .notNull()
...
export const enrichmentAuditLog = leadIntelligenceSchema.table(
