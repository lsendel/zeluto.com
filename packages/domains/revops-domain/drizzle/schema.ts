...
export const deals = revopsSchema.table(
  'deals',
  {
...
export const activities = revopsSchema.table(
  'activities',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const forecasts = revopsSchema.table(
  'forecasts',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
    ...
    rep_id: uuid('rep_id'),
    pipeline_value: numeric('pipeline_value', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    best_case_value: numeric('best_case_value', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    commit_value: numeric('commit_value', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    closed_value: numeric('closed_value', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    weighted_value: numeric('weighted_value', { precision: 12, scale: 2 })
      .notNull()
      .default('0'),
    created_at: timestamp('created_at').notNull().defaultNow(),
    updated_at: timestamp('updated_at').notNull().defaultNow(),
...
export const routingRules = revopsSchema.table(
  'routing_rules',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const sequences = revopsSchema.table(
  'sequences',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
    ...
    >(),
    daily_limits: jsonb('daily_limits').$type<{
      email: number;
      linkedin: number;
      sms: number;
    }>(),
    send_window: jsonb('send_window').$type<{
      start_hour: number;
      end_hour: number;
      timezone: string;
    }>(),
...
export const sequenceEnrollments = revopsSchema.table(
...
export const prospects = revopsSchema.table(
  'prospects',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
    ...
    icp_match: numeric('icp_match', { precision: 5, scale: 2 })
      .notNull()
      .default('0'),
    reasoning: text('reasoning'),
    recommendation: varchar('recommendation', { length: 30 })
      .notNull()
      .default('manual_review'), // enrich, sequence, skip, manual_review
    data_completeness: numeric('data_completeness', { precision: 5, scale: 2 })
      .notNull()
      .default('0'),
  ...
  (table) => ({
    ...
    recommendationIdx: {
      columns: [table.organization_id, table.recommendation],
    },
  }),
);

export const researchJobs = revopsSchema.table(
  'research_jobs',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const researchInsights = revopsSchema.table(
  'research_insights',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const workflows = revopsSchema.table(
  'workflows',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
...
export const workflowExecutions = revopsSchema.table(
  'workflow_executions',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    organization_id: uuid('organization_id').notNull(),
