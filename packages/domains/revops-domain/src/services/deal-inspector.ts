...
export class DealInspector {
  inspect(deal: DealProps, activities: ActivityProps[]): DealHealthReport {
    ...
    if (deal.stage === 'closed_won' || deal.stage === 'closed_lost') {
      return { dealId: deal.id, riskLevel: 'healthy', flags: [], recommendations: [], score: 100 };
    }
    ...
    if (daysSinceActivity >= INACTIVITY_DAYS_CRITICAL) {
      flags.push(`No activity in ${Math.floor(daysSinceActivity)} days (critical)`);
    ...
    const daysInStage = (now - new Date(deal.updatedAt).getTime()) / (1000 * 60 * 60 * 24);
    ...
    if (activities.length >= 5 && activityTypes.size <= 1) {
      ...
      recommendations.push('Add multi-channel touchpoints (call, email, meeting)');
      score -= 10;
    }
