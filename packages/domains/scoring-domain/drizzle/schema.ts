...
export const leadScores = scoringSchema.table('lead_scores', {
  ...
  top_contributors: jsonb('top_contributors').$type<Array<{ factor: string; points: number }>>(),
...
export const scoreHistory = scoringSchema.table('score_history', {
  id: uuid('id').primaryKey().defaultRandom(),
  organization_id: uuid('organization_id').notNull(),
  contact_id: uuid('contact_id').notNull(),
  ...
  orgContactDateIdx: { columns: [table.organization_id, table.contact_id, table.date] },
}));

export const intentSignals = scoringSchema.table('intent_signals', {
  id: uuid('id').primaryKey().defaultRandom(),
  organization_id: uuid('organization_id').notNull(),
  contact_id: uuid('contact_id').notNull(),
  ...
  decay_model: varchar('decay_model', { length: 20 }).notNull().default('linear'), // linear, exponential, step
...
export const signalConfigs = scoringSchema.table('signal_configs', {
  id: uuid('id').primaryKey().defaultRandom(),
  organization_id: uuid('organization_id').notNull(),
  signal_type: varchar('signal_type', { length: 50 }).notNull(),
  weight: numeric('weight', { precision: 5, scale: 2 }).notNull(),
...
export const signalAlerts = scoringSchema.table('signal_alerts', {
  id: uuid('id').primaryKey().defaultRandom(),
  organization_id: uuid('organization_id').notNull(),
  contact_id: uuid('contact_id').notNull(),
  signal_type: varchar('signal_type', { length: 50 }).notNull(),
...
export const scoringConfigs = scoringSchema.table('scoring_configs', {
  id: uuid('id').primaryKey().defaultRandom(),
  organization_id: uuid('organization_id').notNull(),
