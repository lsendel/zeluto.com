...
export class JourneyService {
    ...
    async executeStep(data: {
        ...
        const execution = await this.journeyRepo.findExecutionById(executionId, organizationId);
        ...
        try {
            ...
            switch (step.type) {
                ...
                case 'delay': {
                    ...
                    await this.journeyRepo.logExecution(executionId, organizationId, 'info', `Delay step scheduled: ${delayMs}ms`, { stepId, delayMs });
            ...
            if (step.type === 'action' || step.type === 'trigger') { // Trigger only if starting? but executeStep handles triggers too? No triggers start execution.
                const connections = await this.journeyRepo.findConnectionsFrom(stepId);
            ...
            await this.redis.setex(idempotencyKey, 86400 * 7, '1');
            await this.journeyRepo.logExecution(executionId, organizationId, 'info', `Step executed: ${step.type}`, { stepId, result });
            ...
            await this.journeyRepo.updateStepExecution(stepExec);
            await this.journeyRepo.logExecution(executionId, organizationId, 'error', `Step failed: ${err.message}`, { stepId });
            throw err;
        }
    }

    async processDelayedStep(data: { executionId: string; stepId: string; organizationId: string }): Promise<Result<void>> {
        const { executionId, stepId, organizationId } = data;
        const execution = await this.journeyRepo.findExecutionById(executionId, organizationId);
    ...
    async cleanupStaleExecutions(): Promise<void> {
        ...
        for (const exec of staleExecs) {
            ...
            await this.journeyRepo.logExecution(exec.executionId, exec.organizationId, 'warn', 'Execution canceled due to staleness (stuck > 7 days)');
        }
    }

    async evaluateScoreTriggers(organizationId: string, contactId: string, score: number, eventType: 'LeadScored' | 'IntentSignalDetected', signalType?: string): Promise<void> {
        const triggerType = eventType === 'LeadScored' ? 'score_threshold' : 'intent_signal';

        const triggers = await this.journeyRepo.findTriggersByType(organizationId, triggerType);
        ...
        for (const { trigger, journey } of triggers) {
            ...
            if (triggerType === 'score_threshold') {
                ...
                if (direction === 'up' && score >= minScore) {
                    ...
                    // For now, just log
                    logger.info({ journeyId: journey.journeyId, contactId, score }, 'Score threshold trigger fired');
                ...
                if (!targetSignalType || targetSignalType === signalType) {
                    logger.info({ journeyId: journey.journeyId, contactId, signalType }, 'Intent signal trigger fired');
                }
            }
        }
    }
    ...
    async evaluateSegmentTriggers(): Promise<void> {
        const triggers = await this.journeyRepo.findActiveJourneysWithSegmentTriggers();
        for (const { trigger, journey } of triggers) {
            const config = trigger.config;
            logger.info({ journeyId: journey.journeyId, segmentId: config.segmentId }, 'Segment trigger evaluation placeholder');
        }
    }
    ...
    private calculateDelayMs(config: Record<string, unknown>): number {
        ...
        switch (unit) {
            ...
            default: return duration * 3600 * 1000;
        }
    }

    private evaluateCondition(config: Record<string, unknown>, data?: any): string {
    ...
    private async completeExecution(execution: JourneyExecution): Promise<void> {
        execution.complete();
        await this.journeyRepo.saveExecution(execution);
        await this.journeyRepo.logExecution(execution.executionId, execution.organizationId, 'info', 'Journey execution completed');
    }
}
