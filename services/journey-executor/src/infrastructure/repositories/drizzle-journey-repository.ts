...
export interface IJourneyRepository {
    findExecutionById(executionId: string, organizationId: string): Promise<JourneyExecution | null>;
    saveExecution(execution: JourneyExecution): Promise<void>;

    findStepById(stepId: string, organizationId: string): Promise<JourneyStep | null>;
    findConnectionsFrom(stepId: string): Promise<Array<{ fromStepId: string; toStepId: string; label?: string }>>;

    createStepExecution(stepExecution: StepExecution): Promise<void>;
    updateStepExecution(stepExecution: StepExecution): Promise<void>;

    findTriggersByType(organizationId: string, type: 'score_threshold' | 'intent_signal'): Promise<Array<{ trigger: JourneyTrigger; journey: Journey }>>;
    findActiveJourneysWithSegmentTriggers(): Promise<Array<{ trigger: JourneyTrigger; journey: Journey }>>;

    findStaleExecutions(olderThan: Date): Promise<JourneyExecution[]>;

    logExecution(executionId: string, organizationId: string, level: string, message: string, metadata?: Record<string, unknown>): Promise<void>;
}

export class DrizzleJourneyRepository implements IJourneyRepository {
    constructor(private readonly db: any) { }

    async findExecutionById(executionId: string, organizationId: string): Promise<JourneyExecution | null> {
    ...
    async findStepById(stepId: string, organizationId: string): Promise<JourneyStep | null> {
        const [row] = await this.db
            .select()
            .from(journey_steps)
            .where(
    ...
    async findConnectionsFrom(stepId: string): Promise<Array<{ fromStepId: string; toStepId: string; label?: string }>> {
    ...
    async findTriggersByType(organizationId: string, type: 'score_threshold' | 'intent_signal'): Promise<Array<{ trigger: JourneyTrigger; journey: Journey }>> {
        const rows = await this.db
            .select({
    ...
    async findActiveJourneysWithSegmentTriggers(): Promise<Array<{ trigger: JourneyTrigger; journey: Journey }>> {
    ...
    async findStaleExecutions(olderThan: Date): Promise<JourneyExecution[]> {
        ...
        return rows.map((row: any) => JourneyExecution.reconstitute({
            id: row.id,
            journeyId: row.journey_id,
            journeyVersionId: row.journey_version_id,
            organizationId: row.organization_id,
    ...
    async logExecution(executionId: string, organizationId: string, level: string, message: string, metadata?: Record<string, unknown>): Promise<void> {
