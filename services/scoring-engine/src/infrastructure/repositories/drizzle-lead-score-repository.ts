...
// Helper to map DB row to Domain Entity
...
export class DrizzleLeadScoreRepository implements LeadScoreRepository {
    constructor(private readonly db: NeonHttpDatabase<Record<string, unknown>>) { }
    ...
    async findByOrganization(orgId: string, options?: { minScore?: number; grade?: string; limit?: number; offset?: number }): Promise<LeadScore[]> {
        const conditions = [eq(leadScores.organization_id, orgId)];
        if (options?.minScore) {
            conditions.push(gte(leadScores.total_score, options.minScore));
        }
        if (options?.grade) {
