...
domainRoutes.get('/api/v1/delivery/sending-domains', async (c) => {
  ...
  try {
    ...
    return c.json({ code: 'INTERNAL_ERROR', message: 'Failed to list sending domains' }, 500);
  }
});
...
domainRoutes.post('/api/v1/delivery/sending-domains', async (c) => {
  ...
  try {
    ...
    return c.json({ code: 'INTERNAL_ERROR', message: 'Failed to add sending domain' }, 500);
  }
});
...
domainRoutes.post('/api/v1/delivery/sending-domains/:id/verify', async (c) => {
  ...
  try {
    ...
    return c.json({ code: 'INTERNAL_ERROR', message: 'Failed to verify sending domain' }, 500);
  }
});

// GET /api/v1/delivery/sending-domains/:id/dns-records - Get DNS records
domainRoutes.get('/api/v1/delivery/sending-domains/:id/dns-records', async (c) => {
  const tenant = c.get('tenant');
  const db = c.get('db');
  const id = c.req.param('id');
  ...
  try {
    ...
    return c.json({ code: 'INTERNAL_ERROR', message: 'Failed to get DNS records' }, 500);
  }
});
...
domainRoutes.delete('/api/v1/delivery/sending-domains/:id', async (c) => {
  ...
  try {
    ...
    return c.json({ code: 'INTERNAL_ERROR', message: 'Failed to delete sending domain' }, 500);
  }
});
