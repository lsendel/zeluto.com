...
import { contacts, segments, segment_contacts } from '@mauntic/crm-domain/drizzle';
import type { NeonHttpDatabase } from 'drizzle-orm/neon-http';
import { Contact, type ContactRepository, type ContactProps } from '@mauntic/crm-domain';
...
export class DrizzleContactRepository implements ContactRepository {
    ...
    async findBySegment(
        ...
        const [segment] = await this.db
            ...
            .from(segments)
            .where(and(eq(segments.id, segmentId), eq(segments.organization_id, orgId)))
            .limit(1);
        ...
        if (segment.type === 'static') {
            return this.fetchStaticSegmentContacts(orgId, segmentId, { offset, limit });
        }
    ...
    private async fetchStaticSegmentContacts(
        ...
        const nextOffset = pagination.offset + rows.length < total
    ...
    private async fetchDynamicSegmentContacts(
        ...
        const where = criteria ? buildFilterWhere(criteria, orgId) : eq(contacts.organization_id, orgId);
