...
export class RocketReachAdapter implements EnrichmentProviderAdapter {
  ...
  readonly supportedFields = ['email', 'phone', 'linkedinUrl', 'title', 'company'] as const;
  ...
  async enrich(request: EnrichmentRequest): Promise<EnrichmentAdapterResult> {
    ...
    try {
      ...
      const response = await fetch('https://api.rocketreach.co/api/v2/person/lookup', {
        method: 'POST',
        headers: {
      ...
      if (emails?.[0]) fields.push({ field: 'email', value: emails[0], confidence: 0.85 });
      const phones = data.phones as Array<{ number: string }> | undefined;
      if (phones?.[0]) fields.push({ field: 'phone', value: phones[0].number, confidence: 0.8 });
      if (data.linkedin_url) fields.push({ field: 'linkedinUrl', value: data.linkedin_url, confidence: 0.95 });
      if (data.current_title) fields.push({ field: 'title', value: data.current_title, confidence: 0.85 });
      if (data.current_employer) fields.push({ field: 'company', value: data.current_employer, confidence: 0.85 });

      return { success: true, fields, latencyMs: Date.now() - start, cost: 0.04 };
    } catch (error) {
