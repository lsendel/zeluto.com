...
import {
  ...
  upsertProvider,
} from '../infrastructure/repositories/enrichment-provider-repository.js';
...
providerRoutes.put(
  '/api/v1/lead-intelligence/providers/:providerId',
  async (c) => {
    const tenant = c.get('tenant');
    const db = c.get('db');
    ...
    try {
      ...
      return c.json(
        { code: 'INTERNAL_ERROR', message: 'Failed to configure provider' },
        500,
      );
    }
  },
