...
waterfallRoutes.get('/api/v1/lead-intelligence/waterfall', async (c) => {
  ...
  try {
    const configs = await findWaterfallsByOrganization(
      db,
    ...
    return c.json(
      { code: 'INTERNAL_ERROR', message: 'Failed to list waterfall configs' },
      500,
    );
  }
});
...
waterfallRoutes.put(
  '/api/v1/lead-intelligence/waterfall/:fieldName',
  async (c) => {
    const tenant = c.get('tenant');
    const db = c.get('db');
    ...
    try {
        ...
        return c.json(
          {
            ...
            message: 'providerOrder must have at least 1 entry',
          },
          400,
        );
      }
      ...
      const config = await upsertWaterfall(db, tenant.organizationId, {
        ...
        max_cost_per_lead:
          body.maxCostPerLead != null ? String(body.maxCostPerLead) : null,
      });
      ...
      return c.json(
        { code: 'INTERNAL_ERROR', message: 'Failed to configure waterfall' },
        500,
      );
    }
  },
