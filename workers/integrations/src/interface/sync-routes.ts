...
syncRoutes.post('/api/v1/integrations/connections/:id/sync', async (c) => {
  ...
  try {
      ...
      return c.json(
        {
          ...
          message: `Connection is ${connection.status}, cannot sync`,
        },
        400,
      );
    }
    ...
    return c.json(
      { code: 'INTERNAL_ERROR', message: 'Failed to trigger sync' },
      500,
    );
  }
});
...
syncRoutes.get('/api/v1/integrations/connections/:id/sync-jobs', async (c) => {
  ...
  try {
    ...
    const result = await findSyncJobsByConnection(
      db,
      tenant.organizationId,
      connectionId,
      {
    ...
    return c.json(
      { code: 'INTERNAL_ERROR', message: 'Failed to list sync jobs' },
      500,
    );
  }
});
...
syncRoutes.get('/api/v1/integrations/sync-jobs/:id', async (c) => {
  ...
  try {
    ...
    return c.json(
      { code: 'INTERNAL_ERROR', message: 'Failed to get sync job' },
      500,
    );
  }
});
