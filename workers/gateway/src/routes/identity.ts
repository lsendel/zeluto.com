...
export function createIdentityRoutes() {
  ...
  app.get('/users', async (c) => {
    ...
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/list', {
        page,
        limit,
        search,
  ...
  app.get('/users/:userId', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/get', {
  ...
  app.patch('/users/:userId/profile', async (c) => {
    ...
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/update-profile', {
        userId,
        name: body?.name,
        image: body?.image,
      })) ?? forwardToService(c, c.env.IDENTITY)
  ...
  app.patch('/users/:userId/role', async (c) => {
    ...
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/update-role', {
        userId,
        role: body?.role,
      })) ?? forwardToService(c, c.env.IDENTITY)
    );
  ...
  app.post('/users/:userId/block', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/block', {
        userId: c.req.param('userId'),
      })) ?? forwardToService(c, c.env.IDENTITY)
    );
  });
  ...
  app.post('/users/:userId/unblock', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'users/unblock', {
        userId: c.req.param('userId'),
      })) ?? forwardToService(c, c.env.IDENTITY)
    );
  });
  ...
  app.get('/organizations', async (c) => {
    ...
    const search = searchParams.get('search') ?? undefined;
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/list', {
        page,
        limit,
        search,
      })) ?? forwardToService(c, c.env.IDENTITY)
  ...
  app.post('/organizations', async (c) => {
    ...
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/create', {
        ...payload,
  ...
  app.patch('/organizations/:organizationId', async (c) => {
    ...
    const payload = await parsePayload(c, ['name', 'slug', 'logo']);
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/update', {
        organizationId,
        ...payload,
      })) ?? forwardToService(c, c.env.IDENTITY)
    );
  ...
  app.delete('/organizations/:organizationId', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/delete', {
  ...
  app.post('/organizations/:organizationId/switch', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/switch', {
        organizationId: c.req.param('organizationId'),
      })) ?? forwardToService(c, c.env.IDENTITY)
    );
  });
  ...
  app.get('/organizations/:organizationId/members', async (c) => {
    const searchParams = new URL(c.req.url).searchParams;
    return (
      (await dispatchRequest(c, identityBinding(c), 'organizations/members', {
        organizationId: c.req.param('organizationId'),
  ...
  app.post('/invites/:token/accept', async (c) => {
    return (
      (await dispatchRequest(c, identityBinding(c), 'invites/accept', {
