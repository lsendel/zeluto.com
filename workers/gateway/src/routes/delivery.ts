...
export function createDeliveryRoutes() {
  ...
  app.get('/sending-domains', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'sending-domains/list', {})
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.post('/sending-domains', async (c) => {
    const payload = await parsePayload(c, ['domain']);
    return await dispatchRequest(c, deliveryBinding(c), 'sending-domains/create', payload)
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.post('/sending-domains/:domainId/verify', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'sending-domains/verify', {
      id: c.req.param('domainId'),
    }) ?? forwardToService(c, c.env.DELIVERY);
  });

  app.get('/sending-domains/:domainId/dns-records', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'sending-domains/dns-records', {
      id: c.req.param('domainId'),
    }) ?? forwardToService(c, c.env.DELIVERY);
  });

  app.delete('/sending-domains/:domainId', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'sending-domains/delete', {
      id: c.req.param('domainId'),
    }) ?? forwardToService(c, c.env.DELIVERY);
  });
  ...
  app.get('/jobs', async (c) => {
    ...
    return await dispatchRequest(c, deliveryBinding(c), 'jobs/list', {
      page,
      limit,
  ...
  app.get('/jobs/:jobId', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'jobs/get', {
  ...
  app.get('/jobs/:jobId/events', async (c) => {
    ...
    const page = Number(searchParams.get('page') ?? '1');
    const limit = Number(searchParams.get('limit') ?? '25');
    return await dispatchRequest(c, deliveryBinding(c), 'jobs/events', {
      id: c.req.param('jobId'),
      page,
      limit,
    }) ?? forwardToService(c, c.env.DELIVERY);
  ...
  app.get('/providers', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'providers/list', {})
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.get('/providers/:providerId', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'providers/get', {
  ...
  app.post('/providers', async (c) => {
    ...
    return await dispatchRequest(c, deliveryBinding(c), 'providers/create', payload)
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.patch('/providers/:providerId', async (c) => {
    const payload = await parseDeliveryProviderPayload(c);
    return await dispatchRequest(c, deliveryBinding(c), 'providers/update', {
      id: c.req.param('providerId'),
      ...payload,
    }) ?? forwardToService(c, c.env.DELIVERY);
  });
  ...
  app.delete('/providers/:providerId', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'providers/delete', {
      id: c.req.param('providerId'),
    }) ?? forwardToService(c, c.env.DELIVERY);
  });
  ...
  app.post('/providers/:providerId/test', async (c) => {
    ...
    return await dispatchRequest(c, deliveryBinding(c), 'providers/test', {
      id: c.req.param('providerId'),
      ...payload,
    }) ?? forwardToService(c, c.env.DELIVERY);
  });
  ...
  app.get('/suppressions', async (c) => {
    ...
    const page = Number(searchParams.get('page') ?? '1');
    const limit = Number(searchParams.get('limit') ?? '25');
    return await dispatchRequest(c, deliveryBinding(c), 'suppressions/list', {
  ...
  app.get('/suppressions/check', async (c) => {
    const searchParams = new URL(c.req.url).searchParams;
    return await dispatchRequest(c, deliveryBinding(c), 'suppressions/check', {
  ...
  app.post('/suppressions', async (c) => {
    ...
    return await dispatchRequest(c, deliveryBinding(c), 'suppressions/create', payload)
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.delete('/suppressions/:suppressionId', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'suppressions/delete', {
  ...
  app.get('/warmup', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'warmup/list', {})
      ?? forwardToService(c, c.env.DELIVERY);
  });

  app.get('/warmup/:domainId/progress', async (c) => {
    return await dispatchRequest(c, deliveryBinding(c), 'warmup/progress', {
      id: c.req.param('domainId'),
    }) ?? forwardToService(c, c.env.DELIVERY);
  });
