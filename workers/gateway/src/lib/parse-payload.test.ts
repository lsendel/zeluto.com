...
import { parseJsonBody, parsePayload } from './parse-payload.js';
...
describe('parsePayload', () => {
  it('extracts string fields from JSON body', async () => {
    const c = createMockContext('application/json', {
      name: 'Acme',
      slug: 'acme',
      extra: 123,
    });
  ...
  it('extracts string fields from form data', async () => {
    const c = createMockContext('application/x-www-form-urlencoded', {
      name: 'Acme',
      slug: 'acme',
    });
    const result = await parsePayload(c as any, ['name', 'slug']);
...
describe('parseJsonBody', () => {
  ...
  it('returns null on parse error', async () => {
    const c = {
      req: {
        raw: {
          clone: () => ({ json: () => Promise.reject(new Error('bad')) }),
        },
      },
    };
    const result = await parseJsonBody(c as any);
