  ...
  return (
    <html lang="en" class="h-full bg-gray-50">
      <head>
        ...
        <meta property="og:title" content="Sign In | Zeluto" />
        <link
          rel="stylesheet"
        ...
        <div class="flex min-h-full flex-col items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div class="w-full max-w-md">
            <div class="rounded-xl border border-gray-200 bg-white px-8 py-10 shadow-sm">
              ...
              {/* Email/Password Form - HTMX handles redirect via HX-Redirect header */}
              <form
                hx-post="/api/auth/sign-in/email"
                hx-swap="none"
                hx-indicator=".loading"
                hx-on--after-request="if(!event.detail.successful){document.getElementById('login-error').textContent='Invalid email or password';document.getElementById('login-error').classList.remove('hidden')}else{window.location.href='/app/dashboard'}"
              >
                <div class="space-y-4">
                  <Input
                    name="email"
